...
def fused_leaky_relu(input, bias, negative_slope=0.2, scale=2 ** 0.5):
    return nn.leaky_relu(input + bias, negative_slope) * scale


class FusedLeakyReLU(nn.Module):
    def __init__(self, channel, negative_slope=0.2, scale=2 ** 0.5):
        super().__init__()
        self.bias = jt.zeros(1, channel, 1, 1)  
...
def upfirdn2d_native(
    ...
    out = nn.pad(out, [0, up_x - 1, 0, 0, 0, up_y - 1, 0, 0]) 
...
class EqualLinear(nn.Module):
    def __init__(
        ...
        if bias:
            self.bias = jt.zeros(out_dim) + bias_init  
        else:
            self.bias = None
    ...
    def execute(self, input):
        if self.activation:
            out = nn.linear(input, self.weight * self.scale)  
...
class ScaledLeakyReLU(nn.Module):
    ...
    def execute(self, input):
        out = nn.leaky_relu(input, negative_slope=self.negative_slope)  
...
class ModulatedConv2d(nn.Module):
    ...
    def execute(self, input, style):
        ...
        if style is not None:
            ...
            style = jt.ones(batch, 1, in_channel, 1, 1)  
...
class StyledConv(nn.Module):
    ...
    def execute(self, input, style=None, noise=None):
...
class Generator(nn.Module):
    ...
    def execute( 
        ...
        if len(styles) < 2:
            ...
            latent = jt.cat([latent, latent2], 1) 
...
class StyleGAN2Discriminator(nn.Module):
    ...
    def execute(self, input, get_minibatch_features=False):
        if "patch" in self.opt.netD and self.opt.D_patch_size is not None:
            h, w = input.shape[2], input.shape[3] 
            y = jt.randint(h - self.opt.D_patch_size, ()) 
            x = jt.randint(w - self.opt.D_patch_size, ())
...
class StyleGAN2Encoder(nn.Module):
    ...
    def execute(self, input, layers=[], get_features=False): 
        feat = input
        feats = []
        if -1 in layers:
...
class StyleGAN2Decoder(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, opt=None):
        ...
        for i in range(num_downsampling):
            ...
            convs.append(
                StyledConv(in_channel, out_channel, 3, upsample=True, blur_kernel=blur_kernel, inject_noise=inject_noise)  # 假设StyledConv已转换
            )
            cur_res = cur_res * 2
...
class StyleGAN2Generator(nn.Module):
    ...
    def execute(self, input, layers=[], encode_only=False): 
